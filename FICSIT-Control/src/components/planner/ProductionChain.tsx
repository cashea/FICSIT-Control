...
function formatRate(ratePerMinute: number, cycleDuration: number) {
  const perCycle = (ratePerMinute / 60) * cycleDuration;
  return `${ratePerMinute.toFixed(1)}/min (${perCycle.toFixed(1)}/cycle)`;
}
                      ...
                      {node.inputs.map((inp) => (
                        ...
                        >
                          ...
                          <span className="font-mono text-[var(--color-satisfactory-text-dim)]">
                            {formatRate(inp.ratePerMinute, cycleDuration)}
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="space-y-0.5">
                    {node.outputs.map((out) => (
                      ...
                      >
                        ...
                        <span className="font-mono text-[var(--color-satisfactory-text-dim)]">
                          {formatRate(out.ratePerMinute, cycleDuration)}
                        </span>
                      </div>
                    ))}
                  </div>
